...
export const AiCameraModal: React.FC<AiCameraModalProps> = ({
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md transition-opacity duration-300 ${isExiting ? 'opacity-0 pointer-events-none' : 'animate-in fade-in'}`}>
      <div className={`bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden border border-slate-100 flex flex-col h-[82vh] sm:h-[75vh] max-h-[640px] transition-transform duration-300 ${isExiting ? 'scale-95 translate-y-10' : 'scale-100'}`}>
        {/* Header */}
